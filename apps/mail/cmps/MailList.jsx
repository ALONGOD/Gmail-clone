        ...
        {emails.map(email => {
          ...
          return (
            <li key={email.id} className={`${isReadClass} mail flex flex-row align-center`} onMouseEnter={() => setHoverMailId(email.id)} onMouseLeave={() => setHoverMailId(null)}>
              <MailPreview key={email.id} setMailMainContent={setMailMainContent} email={email} hoverMailId={hoverMailId} onRemove={onRemove} onToggleIsStar={onToggleIsStar} onToggleIsRead={onToggleIsRead} />
            </li>
          )
        })}
      </ul>
